...
```{r gdp-long }
# Show GDP growth change for each industry

# Create subgroups for the sectors
sg_gdp_long <- sg_gdp %>%
  pivot_longer(cols = manufacturing:ownership_of_dwellings,
               names_to = "gdp_goods_services",
               values_to = "values_goods_services") %>%
  # Categorise each sector according to their respective industry
  mutate(industry = case_when(
    gdp_goods_services %in% c("manufacturing", "construction", "utilities", "other_goods_industries") ~ "goods",
    gdp_goods_services %in% "ownership_of_dwellings" ~ "ownership",
    TRUE ~ "services")
    )
