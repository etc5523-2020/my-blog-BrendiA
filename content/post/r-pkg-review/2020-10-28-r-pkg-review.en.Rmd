...
The intended purpose of this shiny package is to provide an embedded interactive Shiny application within the package that allow users to explore a Covid-19 related data. To be more specific, the R package should have an exported function `launch_app()` which load the aforementioned Shiny application; along with some functions that
refactors the user interface or server-side functions within the application.

## Did it do it?

According to the `README` file, the goal of this package is to analyse Covid-19 cases across countries. This package may also be installed via the code:

```{r, echo = TRUE, eval = FALSE}
devtools::install_github("etc5523-2020/r-package-assessment-AntiRosa")
```

When I tried running the code above, it could not run. Accordingly, I looked to examine the package's repository. In trying to rectifying this error, I learnt that the package could not be built/installed (*i.e.* I
ran into errors when I ran the code `devtools::install()` in the repository). Furthermore, there were a few examples of the codes found in the README file were not related to a Covid-19 dataset.
...
<div class="clearfix">
  ...
  <img src = "../images/dir_tree1.png" width="120%" height="150%"/>
  </div>
  <div class="img-container">
  <img src = "../images/dir_tree2.png" width="120%" height="150%"/>
  </div>
</div>

Two `.Rproj` were enclosed in the working directory, one in the main working directory (`r-package-assessment-AntiRosa.Rproj`) and the other in the `covdata` directory. I attempted to resolve the issues related to building the R package in both .Rproj, but to no avail. However, The `covdata` .Rproj seemed to be more comprehensive than the other R project. For this reason, I will examine the R package related to `covdata.Rproj`.

## Are there improvements that could be made to the code?

Although the R package was not available for use, the package structure followed the general procedures, such as having R functions that you want in the package to be in the R directory, as shown above in the working directory tree.
...
There were minimal documentation made in this package and there were no exported functions.

## What I have Learned from Reviewing

-   It is is good practice to run `devtools::check()` from time to time to ensure that functions are working and the package may be loaded properly. Debugging may be a chore if we do not do this!
...
-   README file should highlight how users can access the package, requirements needed to use the package, and how users can use it.
